...
#define EEPROM_ACTION_NUM_ADDR 16u   /* 存放动作组内的动作个数(store the number of acions in the action group) */
...
static uint8_t knob_angles[6] = { 90, 90, 90, 90, 90, 90 };   /* 旋钮产生的角度数值(angle values generated by the knob) */
...
static float servo_angles[6] = { 90, 90, 90, 90, 90, 90 };  /* 舵机实际控制的角度数值(the actual angle values controlled by servos) */
...
static void knob_update(void);   /* 旋钮读取更新(update knob read) */
...
void loop() {
  ...
  tune_task();
  // 按键扫描及其动作实现(button scanning and action achievement)
  key_scan();
  // 旋钮读取更新(update knob read)
  knob_update();
  // 舵机控制(servo control)
  servo_control();
...
void key_scan(void) { /* 按键扫描(button scanning) */
