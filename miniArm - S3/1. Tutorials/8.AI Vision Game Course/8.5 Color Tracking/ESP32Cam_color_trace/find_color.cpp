...
{
  {
    ...
    find_blobs_list_lnk_data_t lnk_blob,max_blob = {.pixels = 0},tmp_blob;
    list_t out_temp;
    list_init(&out_temp, sizeof(find_blobs_list_lnk_data_t));
    uint8_t color = 0;
    ...
    if(list_size(out) > 1)
    {
      ...
      {
        list_pop_front(out, &tmp_blob);
        if(lnk_blob.code == tmp_blob.code)
        {
          if(lnk_blob.pixels < max_blob.pixels)
          {
              memcpy(&lnk_blob, &tmp_blob, sizeof(find_blobs_list_lnk_data_t));
          }
        }else{
          list_push_back(&out_temp , &lnk_blob);
          memcpy(&lnk_blob, &tmp_blob, sizeof(find_blobs_list_lnk_data_t));
