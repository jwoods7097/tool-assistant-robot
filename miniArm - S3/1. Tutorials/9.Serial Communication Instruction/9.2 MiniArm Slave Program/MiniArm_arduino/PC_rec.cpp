...
/* 引脚定义(define pins) */
const static uint8_t servoPins[6] = { 7, 6, 5, 4, 3 ,2};//舵机引脚定义(define servo pins)

const static uint8_t Buzzer_Pin = 11; // 蜂鸣器引脚定义(define buzzer pin)
const static uint8_t rgbPin = 13; //RGB灯引脚定义(define RGB LED pin)
...
const uint8_t limt_angles[6][2] = {{0,180},{0,180},{0,180},{0,180},{0,180},{0,180}}; /* 各个关节角度的限制(the angle limit for each joint) */
static float servo_angles[6] = { 90,90,90,90,90,90};  /* 舵机实际转动角度数值(servo's actual rotation angle values) */
static uint8_t extended_func_angles[6] = { 90,90,90,90,90,90}; //动作数据(action data)
...
{
  SerialM.begin(9600);
  //舵机初始化(initialize servos)  
  for(int i= 0;i<6;i++){
    servos[i].attach(servoPins[i],500,2500);
  }
  //蜂鸣器PWM初始化(buzzer PWM initialization)
  pinMode(Buzzer_Pin,OUTPUT);

  //RGB灯初始化(initialize RGB LED)  
  ...
  while(pk_ctl.len > 0)
  {
    switch(pk_ctl.state)
    {
      ...
      case STATE_FUNCTION: /* 处理帧功能号(process frame function number) */
        ...
        break;
      case STATE_LENGTH: /* 处理帧数据长度(process frame data length) */
        if(pk_ctl.data[pk_ctl.index_head] >= DATA_SIZE) //若（包含具体信息）信息数据长度>DATA_SIZE,则有问题(If the length of the information data exceeds the value of DATA_SIZE, it indicates a problem)
        {
          pk_ctl.state = STATE_STARTBYTE1;
          continue;
        }else{
      ...
      case STATE_DATA: /* 处理帧数据(process frame data) */
        ...
        break;
      case STATE_CHECKSUM: /* 处理校验值(process checksum value) */
        ...
        Serial.print("crc:");
        Serial.print(crc);
        if(crc == pk_ctl.frame.checksum) { /* 校验失败, 跳过执行(If the checksum is failed, jump executing) */
            Serial.println(" OK");
            deal_command(&pk_ctl.frame); //处理数据(process data)
        }else{
          Serial.print("not:");
          Serial.println(pk_ctl.frame.checksum);
        }
        memset(&pk_ctl.frame, 0, sizeof(struct PacketRawFrame)); //清除(clear)
        pk_ctl.state = STATE_STARTBYTE1;
        break;
      default:
        pk_ctl.state = STATE_STARTBYTE1;
...
void PC_REC::deal_command(struct PacketRawFrame* ctl_com)
{
  ...
  {
    case FUNC_SET_SERVO: //设置舵机 0x01(set servo 0x01)
      {
    ...
    case FUNC_READ_ANGLE: //读取角度 0x11(read angle 0x01)
      {
        ...
        uint8_t send_data[20] = {0xAA , 0x77 , 0x11 , 0x06};
        memcpy(&send_data[4] , angles , 6);
        send_data[10] = checksum_crc8(&send_data[2] , 9);
    ...
    case FUNC_SET_BUZZER://设置蜂鸣器 0x02(set buzzer 0x02)
      {
    ...
    case FUNC_SET_RGB://设置RGB灯 0x03(set RGB LED 0x03)
      {
