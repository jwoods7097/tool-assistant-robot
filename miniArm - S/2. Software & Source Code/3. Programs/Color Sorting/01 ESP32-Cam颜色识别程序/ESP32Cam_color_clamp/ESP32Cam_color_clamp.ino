...
void setup() {
  ...
  ledcSetup(10, 1000, 8);  //设置LEDC通道8频率为1，分辨率为8位，即占空比可选0~255
  ledcAttachPin(4, 10); //设置LEDC通道8在IO4上输出
  ledcWrite(10, 0); //设置输出PWM
...
void loop() {
  ...
  if(lightness)
    ledcWrite(10, lightness); //设置输出PWM
  else
    ledcWrite(10, 0);
...
void receiveEvent(int howMany) {
  ...
  while (0 < Wire.available()) { // loop through all but the last
    ...
    {
      case 0:
        if(rd < COLOR_NUM) //读取颜色识别数据，需装载数据
        {
          read_num = rd;
        }else if(rd == 0x11) //控制闪光灯亮灭，跳到第二步
        {
          step = 1;
          read_num = 0xFF;
        }
